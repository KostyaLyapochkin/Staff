...
class SwipeController(private val onLeftClicked: ((position: Int) -> Unit)?,
                      private val onRightClicked: ((position: Int) -> Unit)?) : Callback() {
    ...
    private fun setTouchUpListener(c: Canvas, recyclerView: RecyclerView, viewHolder: RecyclerView.ViewHolder, dX: Float, dY: Float, actionState: Int, isCurrentlyActive: Boolean) {
        recyclerView.setOnTouchListener { _, event ->
            if (event.action == MotionEvent.ACTION_UP) {
                ...
                if (onRightClicked != null && onLeftClicked != null && buttonInstance != null && buttonInstance!!.contains(event.x, event.y)) {
                    if (buttonShowedState === ButtonsState.LEFT_VISIBLE) {
                        onLeftClicked.invoke(viewHolder.adapterPosition)
                        ...
                        onRightClicked.invoke(viewHolder.adapterPosition)
                    }
                }
